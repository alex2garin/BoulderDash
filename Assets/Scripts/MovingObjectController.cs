...
public class MovingObjectController : MonoBehaviour {
	...
	void Update ()
    {
        if (!IsMoving)
        {
            ...
            //Collider2D down = Physics2D.OverlapPoint(transform.position + new Vector3(0f, -1f, 0f));
            if (down == null)
            {
            ...
            if (movingObject.IsMoving == true) return;


            //Collider2D left = Physics2D.OverlapPoint(transform.position + new Vector3(-1f, 0f, 0f));
            Collider2D left = Physics2D.OverlapBox(transform.position + new Vector3(-1f, 0f, 0f), new Vector2(.9f, .9f), 0f);
            if (left == null)
            {
                //Collider2D leftDown = Physics2D.OverlapPoint(transform.position + new Vector3(-1f, -1f, 0f));
            ...
            //Collider2D right = Physics2D.OverlapPoint(transform.position + new Vector3(1f, 0f, 0f));
            Collider2D right = Physics2D.OverlapBox(transform.position + new Vector3(1f, 0f, 0f), new Vector2(.9f, .9f), 0f);
            if (right == null)
            {
                //Collider2D rightDown = Physics2D.OverlapPoint(transform.position + new Vector3(1f, -1f, 0f));
